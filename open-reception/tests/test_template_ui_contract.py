...
def test_file_picker_entry_points_use_onclick_pattern():
    for template in TEMPLATES:
        html = template.read_text(encoding="utf-8")
        assert html.count('id="qaFileInput"') == 1
        assert html.count('id="fileInput"') == 1
        # QA attach button: <button onclick=...> (iOS Safari compatible)
        assert 'class="qa-attach-btn" onclick="document.getElementById(\'qaFileInput\').click()"' in html
        assert '<label class="qa-attach-btn"' not in html
        # File drop zone: div onclick (iOS Safari compatible)
        assert 'id="fileDropZone" onclick="document.getElementById(\'fileInput\').click()"' in html
        assert 'id="fileDropZone" aria-label=' not in html
        # Hidden native input
        assert 'id="fileInput" style="display:none"' in html
        assert 'onchange="handleQaFileSelect(event)"' in html
        assert 'onchange="handleFileSelect(event)"' in html
        # No broken overlay/label pattern
        assert 'class="native-file-input"' not in html
        assert '.native-file-input {' not in html
        assert 'clip-path: inset(50%)' not in html
        assert 'opacity: 0.001' not in html

def test_mobile_file_inputs_have_no_overlay_css():
    """Overlay CSS (opacity:0.001 / clip-path:inset) breaks iOS Safari - verify it is absent."""
    for template in TEMPLATES:
        html = template.read_text(encoding="utf-8")
        assert '.qa-attach-btn .native-file-input {' not in html
